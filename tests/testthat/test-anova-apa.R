...
if (requireNamespace("ez", quietly = TRUE)) {
  ...
  test_that("Output for anova_apa: oneway between ANOVA (ez)", {
    ...
    expect_identical(`[.data.frame`(anova_ez, anova_ez$effect == "dose",
                                    "text"),
                     "F(2, 12) = 5.12, p = .025, petasq = .46")

  })

  test_that("Output for anova_apa: factorial between ANOVA (ez)", {
    ...
    expect_identical(`[.data.frame`(anova_ez,
                                    anova_ez$effect == "gender:alcohol",
                                    "text"),
                     "F(2, 42) = 11.91, p < .001, petasq = .36")

  })

  test_that("Output for anova_apa: repeated-measures ANOVA (ez)", {
    ...
    data <- data.frame(
      ...
      retch = c(8, 7, 1, 6, 9, 5, 2, 5, 6, 2, 3, 8, 5, 3, 1, 9, 8, 4, 5, 8, 7,
                5, 6, 7, 10, 2, 7, 2, 12, 6, 8, 1)
    )
  ...
  test_that("Output for anova_apa: factorial repeated-measures ANOVA (ez)", {
    ...
    data <- data.frame(
      ...
      attitude = c(1, 6, 5, 38, -5, 4, 10, -14, -2, 43, 30, 8, 20, -12, 4, 9,
                   -10, -13, 15, 15, 12, 20, -15, 6, 6, -16, 1, 40, 30, 19, 28,
                   -4, 0, 20, -10, 2, 8, 12, 8, 11, -2, 6, 27, 5, -5, 17, 17,
                   15, 17, -6, 6, 9, -6, -13, 30, 21, 21, 15, -2, 16, 19, -20,
                   3, 34, 23, 28, 27, -7, 7, 12, -12, 2, 34, 20, 26, 24, -10,
                   12, 12, -9, 4, 26, 27, 27, 23, -15, 14, 21, -6, 0, 1, -19,
                   -10, 28, -13, 13, 33, -2, 9, 7, -18, 6, 26, -16, 19, 23, -17,
                   5, 22, -8, 4, 34, -23, 14, 21, -19, 0, 30, -6, 3, 32, -22,
                   21, 17, -11, 4, 40, -6, 0, 24, -9, 19, 15, -10, 2, 15, -9, 4,
                   29, -18, 7, 13, -17, 8, 20, -17, 9, 30, -17, 12, 16, -4, 10,
                   9, -12, -5, 24, -15, 18, 17, -4, 8, 14, -11, 7, 34, -14, 20,
                   19, -1, 12, 15, -6, 13, 23, -15, 15, 29, -1, 10)
    )
    ...
    expect_identical(`[.data.frame`(anova_ez, anova_ez$effect == "drink",
                                    "text"),
    ...
    expect_identical(`[.data.frame`(anova_ez,
                                    anova_ez$effect == "drink:imagery", "text"),
                     "F(4, 76) = 17.15, p < .001, petasq = .47")

  })

  test_that("Output for anova_apa: mixed ANOVA (ez)", {
    ...
    data <- data.frame(
      ...
      rating = c(86, 84, 67, 88, 69, 50, 97, 48, 47, 91, 83, 53, 83, 74, 48, 86,
                 ...
                 96, 63, 42, 92, 48, 43, 89, 85, 61, 87, 79, 44, 86, 50, 45,
                 100, 94, 56, 86, 71, 54, 84, 54, 47, 90, 74, 54, 92, 71, 58,
                 78, 38, 45, 89, 86, 63, 80, 73, 49, 91, 48, 39, 89, 91, 93, 88,
                 65, 54, 55, 48, 52, 84, 90, 85, 95, 70, 60, 50, 44, 45, 99,
                 100, 89, 80, 79, 53, 51, 48, 44, 86, 89, 83, 86, 74, 58, 52,
                 48, 47, 89, 87, 80, 83, 74, 43, 58, 50, 48, 80, 81, 79, 86, 59,
                 47, 51, 47, 40, 82, 92, 85, 81, 66, 47, 50, 45, 47, 97, 69, 87,
                 95, 72, 51, 45, 48, 46, 95, 92, 90, 98, 64, 53, 54, 53, 45, 95,
                 93, 96, 79, 66, 46, 52, 39, 47)
    )
    ...
    expect_identical(`[.data.frame`(anova_ez, anova_ez$effect == "looks",
                                    "text"),
  ...
  test_that("anova_apa: markdown (ez)", {
  ...
  test_that("anova_apa: rmarkdown (ez)", {
  ...
  test_that("anova_apa: plotmath (ez)", {
...
if (requireNamespace("afex", quietly = TRUE)) {
  test_that("Output for anova_apa: oneway between ANOVA (afex)", {
  ...
  test_that("Output for anova_apa: factorial between ANOVA (afex)", {
  ...
  test_that("Output for anova_apa: repeated-measures ANOVA (afex)", {
  ...
  test_that("Output for anova_apa: factorial repeated-measures ANOVA (afex)", {
  ...
  test_that("Output for anova_apa: mixed ANOVA (afex)", {
    ...
    expect_identical(`[.data.frame`(anova_afex,
                                    anova_afex$effect == "gender:looks",
                                    "text"),
                     "F(2, 36) = 80.43, p < .001, petasq = .82")
  ...
  test_that("anova_apa: markdown (afex)", {
  ...
  test_that("anova_apa: rmarkdown (afex)", {
  ...
  test_that("anova_apa: latex (afex)", {
  ...
  test_that("anova_apa: plotmath (afex)", {
